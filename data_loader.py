...
class SalObjDataset(Dataset):
    def __init__(self, images_paths, masks_path, transform=None):
        # self.root_dir = root_dir
        # self.image_name_list = glob.glob(image_dir+'*.png')
        # self.label_name_list = glob.glob(label_dir+'*.png')
        self.image_name_list = images_paths
        self.label_name_list = masks_path
...
def make_affine(image, mask):
    ...
    res_img = T.functional.affine(image, angle=angle, translate=translate, scale=scale, shear=shear)
    res_mask = T.functional.affine(mask, angle=angle, translate=translate, scale=scale, shear=shear)
...
def random_color_jitter(image, mask):
    ...
    res = T.ColorJitter(brightness=0.3, hue=0.3)(image)
    return res, mask
...
class TextSegDataset(Dataset):
    ...
    def __getitem__(self, idx):
        image = torch.from_numpy(io.imread(self.images_paths[idx]).astype(np.float32) / 255)
        mask = torch.from_numpy(io.imread(self.masks_paths[idx]).astype(np.float32) / 255)

        image = torch.transpose(image, 0, 2)
        mask = torch.stack([mask, mask, mask], 2).transpose(0, 2)
